...
/**
 * Test property change events. Set up at <code>TestBean</code> class who emits
 * property change events using standard bean property change mechanisms. Then
 * use a custom JFsm property change event,
 * <code>MyBeanPropertyChangeEvent</code> to catch these and input to the FSM.
 * The <code>IntGt</code> guard condition test the changed value and says 'true'
 * when the value greater than 5 has been set using the <code>setX()</code>
 * method.
 * 
 * @author rwe
 */
