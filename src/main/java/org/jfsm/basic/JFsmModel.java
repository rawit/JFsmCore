...
/**
 * Contains the elements that make up a state machine: - one or more states - a specified start state.
 * 
 */
public class JFsmModel implements JFsmModelI {
    ...
    /**
     * Validates a given machine, by checking for - presence of at least one state - presence of at least one transition
     * - states in transitions are present in states list. If no exception is thrown, the JFsm Model can be run by the
     * FsmEngine (Should check for cycles of transitions with no events)
     * 
     ...
     * 
     *@return true if at least one of the states has a entry action, otherwise false
     */
